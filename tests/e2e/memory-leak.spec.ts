...
test("memory leak — heap growth stays under budget over a long-running room", async ({
  ...
  await ctx.addInitScript(
    ...
    { prefix: "mesh-improv-director", room: `leak-${Date.now()}` },
  );

  const a = await ctx.newPage();
  const b = await ctx.newPage();
  await Promise.all([
    a.goto("/mesh-improv-director/", { waitUntil: "domcontentloaded" }),
    b.goto("/mesh-improv-director/", { waitUntil: "domcontentloaded" }),
  ]);
